...
class Bai2_UDP_ThreadSocket extends Thread {
	DatagramSocket serverSocket = null;
	public Bai2_UDP_ThreadSocket(DatagramSocket serverSocket) {
	...
	public void run() {
		...
		try {
			...
			while (true) {
				...
				if (!st.trim().equals("")) {
					sendData = ("Kết quả: " + c.valueMath(st)).getBytes();
				} else {
...
public class Bai2_UDP_S extends JFrame implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == btnRun) {
			try {
				...
				while (true) {
					new Bai2_UDP_ThreadSocket(serverSocket).start();
				}
			} catch (Exception ex) {
	...
	public static void main(String[] args) throws Exception {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					Bai2_UDP_S frame = new Bai2_UDP_S();
